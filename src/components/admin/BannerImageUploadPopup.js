...
export default function BannerImageUploadPopup({ banner, onClose, onUpdate }) {
  const [name, setName] = useState("");
  const [alt, setAlt] = useState(banner.alt || "");
  const [file, setFile] = useState(null);
  const [loading, setLoading] = useState(false);

  // On mount, prefill the name field by extracting current filename (without extension)
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    reader.onloadend = async () => {
      // Extract only the base64 data (remove data URI prefix)
      ...
      try {
        const res = await fetch(`/api/bannerImage/homepage`, {
          method: "POST",
          headers: {
