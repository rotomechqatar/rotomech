...
export async function POST(request, { params }) {
  try {
    ...
    // Construct the file path dynamically based on the page
    ...
    const putResponse = await fetch(
      `https://api.github.com/repos/${owner}/${repo}/contents/${filePath}`,
      {
        ...
        body: JSON.stringify({
          message: `Upload image ${filename} for ${page} via admin panel`,
          content: fileData, // fileData should be Base64-encoded already
          branch,
        }),
      }
    );
