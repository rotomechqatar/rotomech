...
import AddClientPopup from "@/components/admin/AddClientPopup"; // New AddClientPopup component
...
export default function AdminHomepage() {
  ...
  useEffect(() => {
    fetch("/api/getContent/homepage")
      .then((res) => res.json())
      .then((data) => {
        console.log("Fetched homepage data:", data); // For debugging
        setData(data);
      })
  ...
  // Update client logos by adding the new logo to the correct type group
  const updateClientLogos = (newClient) => {
    // newClient should have: { type, name, imagePath }
    setData((prev) => {
      const newClientData = { ...prev.clientData };
      if (!newClientData[newClient.type]) {
        newClientData[newClient.type] = {};
      }
      newClientData[newClient.type][newClient.name] = newClient.imagePath;
      return { ...prev, clientData: newClientData };
    });
  };

  // Remove a client logo from the state by type and name
  const removeClientLogo = (type, clientName) => {
    setData((prev) => {
      const newClientData = { ...prev.clientData };
      if (newClientData[type]) {
        delete newClientData[type][clientName];
      }
      return { ...prev, clientData: newClientData };
    });
  };
  ...
  const handleDeleteClient = async (type, clientName) => {
    if (!confirm("Are you sure you want to delete this client?")) return;
    try {
      const res = await fetch("/api/client/deleteClient", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type, name: clientName }),
      });
      const result = await res.json();
      if (res.ok) {
        removeClientLogo(type, clientName);
        showGlobalMessage("Client deleted successfully!", "success");
      } else {
        showGlobalMessage(
          "Error deleting client: " + (result.error || "Unknown error"),
          "error"
        );
      }
    } catch (err) {
      console.error(err);
      showGlobalMessage("Error deleting client: " + err.message, "error");
    }
  };
    ...
    <div className="admin-dashboard p-6 bg-gray-50 min-h-screen">
      ...
      {showAddClientPopup && (
        <AddClientPopup
          ...
          onUpdate={(newClient) => {
            updateClientLogos(newClient);
            setShowAddClientPopup(false);
            showGlobalMessage("Client added successfully!", "success");
          }}
        />
      )}
    </div>
...
// Updated ClientLogosSection that uses nested clientData, shows the type column, and adds delete functionality.
function ClientLogosSection({ clientData, onDelete, onAdd }) {
  ...
  return (
    <section className="mb-8 p-6 bg-white rounded shadow">
      ...
      <ImageTable
        images={logos}
        showType={true}
        onDelete={(img) => onDelete(img.type, img.name)}
      />
    </section>
  );
}
...
// -----------------------
// Other Sections (Banner, Legacy, Director Message, CTA, SEO)
// -----------------------
